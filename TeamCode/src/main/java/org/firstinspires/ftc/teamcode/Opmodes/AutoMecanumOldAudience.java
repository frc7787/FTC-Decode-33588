...
@Autonomous(name="Auto Audience OLD - use if pinpoint not working", group="StarterBot")
//@Disabled
public class AutoMecanumOldAudience extends OpMode
{
     ...
     */
    final double LAUNCHER_TARGET_VELOCITY = 1140;  // 1125
    final double LAUNCHER_MIN_VELOCITY = 1090; // 1075
    ...
    double INCHES_TO_DRIVE_FROM_GOAL = 28;  // inches to drive from goal after launching balls in basic auto config
    double MOVE_FORWARDS = 47;
    double  ROBOT_ROTATION_ANGLE = 55;
    double robotRotationAngle;
    ...
    private enum DriveDirection {
        FORWARD, REVERSE, STRAFERIGHT, STRAFELEFT, ROTATERIGHT, ROTATELEFT, STOP
    }
    ...
    public void loop() {
        ...
        switch (autonomousState){
            ...
            case WAIT_FOR_LAUNCH:
                ...
                if(launch(false)) {
                    ...
                    if(shotsToFire > 0) {
                        ...
                        initializeDrive(DriveDirection.REVERSE);
            ...
            case DRIVING_AWAY_FROM_GOAL:
                ...
                if(driving(1)){
                    ...
                    if(alliance == Alliance.RED) {
                        initializeDrive(DriveDirection.ROTATELEFT); // rotate one way
                    } else if (alliance == Alliance.BLUE) {
                        initializeDrive(DriveDirection.ROTATERIGHT);  // rotate the other way
                    }
                    autonomousState = AutonomousState.ROTATING;
                }
                break;
            ...
            case ROTATING:
                if (driving(0.15)) {
                    if (alliance == Alliance.BLUE) {
                        initializeDrive(DriveDirection.STRAFELEFT);
                        autonomousState = AutonomousState.DRIVING_OFF_LINE;
                    } else {
                        initializeDrive(DriveDirection.STRAFERIGHT);
                        autonomousState = AutonomousState.DRIVING_OFF_LINE;
                    }
                }
            ...
            case DRIVING_OFF_LINE:
                if(driving(0.75)){
                    initializeDrive(DriveDirection.STOP);
                    autonomousState = AutonomousState.COMPLETE;
                }
                break; //used to be _26 instead of move forwards
        ...
        telemetry.addData("drive time", driveTimer.seconds());
        /*
    ...
    public void initializeDrive(DriveDirection driveThisWay) {
        driveTimer.reset();
        if (driveThisWay == DriveDirection.FORWARD) {
            drive.drive(0.5,0.0,0.0);
        } else if (driveThisWay == DriveDirection.REVERSE) {
            drive.drive(-0.5,0.0,0.0);
        } else if (driveThisWay == DriveDirection.STRAFERIGHT) {
            drive.drive(0.0,0.5,0.0);
        } else if (driveThisWay == DriveDirection.STRAFELEFT) {
            drive.drive(0.0,-0.5,0.0);
        } else if (driveThisWay == DriveDirection.ROTATERIGHT) {
            drive.drive(0.0,0.0,0.5);
        } else if (driveThisWay == DriveDirection.ROTATELEFT) {
            drive.drive(0.0,0.0,-0.5);
        } else if (driveThisWay == DriveDirection.STOP) {
            drive.drive(0,0,0);
        }
    }

    boolean driving(double timeSeconds) {
        if (driveTimer.seconds() > timeSeconds) {
            telemetry.addData("END DRIVING","TRUE");
            return true;
        } else {
            telemetry.addData("END DRIVING","FALSE");
            return false;
        }
    }
    boolean driveOLD(double speed, double distance, DistanceUnit distanceUnit, double holdSeconds) {
